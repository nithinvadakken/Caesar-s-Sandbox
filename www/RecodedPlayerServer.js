...
BULLET.prototype = {
  ...
  //                  console.log("deleted1")
            ...
            if(temp === 0){
                console.log("bad")
                console.log(this.p2,this.t2,this.i2)
            }
            if(PLAYERS[this.p2].army[this.t2] === undefined){
                console.log(this.p2,this.t2,this.i2)
            }
    ...
    updatepos(){
        ...
        if (t2 === -1 || i2 === -1 ) {
            ...
            for (let i = 0; i < BULLETS.length; i++) {
                if (this.id === BULLETS[i].id) {
                    console.log("enemy died so no bullet")
                    BULLETS.splice(i, 1);
                    temp = 1;
                }
...
class temp {
    ...
    attack(player, troop, index, Eplayer, Etroop, Eindex) {
        ...
        if (PLAYERS[player].army[troop].dates[index] + GAMETROOPATTACKSPDS[troop] * 1000 < d.getTime()) {
            ...
            console.log(player, troop, index, Eplayer, Etroop, Eindex)
    ...
    createArmy(x,y,type,Ex,Ey,Etype) {
        ...
        for (let i = 0; i < x.length; i++) {//TODO make this easier make all of them into 3 arrays x,y,type
            //x, y, type, player
            console.log(x[i], y[i], type[i], 0);
            this.addtroop(x[i], y[i], type[i], 0)
        }
        for (let i = 0; i < Ex.length; i++) {//TODO make this easier make all of them into 3 arrays x,y,type
            //x, y, type, player
            this.addtroop(Ex[i], Ey[i], Etype[i], 1)
        }
        console.log("hi"+ PLAYERS)
    }
